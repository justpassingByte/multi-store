...
export const CellAction =({data}: CellActionProps) =>{
    ...
    const onDelete = async () => {
      ...
      try {
          ...
          console.log("Attempting to delete from Firebase Storage:", data.imageUrl);
          
          // Ensure the storage reference is valid and the image gets deleted
          await deleteObject(ref(storage, data.imageUrl));

          // Log the API call to delete the billboard in Firestore
          console.log(`Attempting to delete from Firestore: /api/${params.storeId}/billboards/${data.id}`);
  ...
  const onCopy = () =>{
    ...
    toast.success("Bill board label copied to clipboard")
  }
    return (
        <div>
             <AlertModal
